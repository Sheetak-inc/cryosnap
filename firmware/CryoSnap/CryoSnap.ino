...
static void _pd_reinit() {
  ...
  // Release the PD-budget clamp (latched in section 3 of task_100ms
  // when measured power crossed g_pd_budget_mW). tps_init above
  // already reset the chip's V/I registers, so any stale clamp the
  // firmware was tracking is meaningless now. Operator re-enable
  // is the intended release path. (BUG-004 / C-1 fix.)
  g_pd_clamped           = false;
}
...
static void task_100ms() {
  ...
  if (_tried_drive_last_tick && g_vmax_mV > SUPPLY_VLIM_FLOOR && !g_pd_clamped) {
    ...
    // Not driving (or clamp latched) — chip's V_limit reading is
    // meaningless for the supply check. Reset so a fresh enable
    // starts the assessment from zero.
    _supply_fault_count = 0;
  }
#endif
